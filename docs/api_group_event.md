...
| `mission_attempt` | optional | int | A number that indicates the count of the number of attempts *within the group attempt* to complete a mission. It can be used to group and order the events. **NOTE**: with the availability of a `group_attempt` and `grouping_code` this field can probably be removed. |
